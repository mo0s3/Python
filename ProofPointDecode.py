# a quick wrapper I wrote for the script from https://help.proofpoint.com/@api/deki/files/177/URLDefenseDecode.py?revision=3
# all code in the URLDefenseDecoder class was from there
...
class URLDefenseDecoder:
    ...
    def __init__(self):
        ...
        URLDefenseDecoder.v1_pattern = re.compile(r'u=(?P<url>.+?)&(amp;)?k=')
        URLDefenseDecoder.v2_pattern = re.compile(r'u=(?P<url>.+?)&(amp;)?[dc]=')
...
def decode(url):
    ...
    try:
        return urldefense_decoder.decode(url)
    except:
        return False

encoded_url = input('ProofPoint Encoded URL:\n')
